      ...
      <section className="bg-gradient-to-b from-[#F7FAF9] to-white py-14 md:py-20">
        <div className="container-padded text-center">
            ...
            <div className="flex items-center gap-1.5">
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={16} className="fill-[#C9A45C] text-[#C9A45C]" aria-hidden />
                ))}
              </div>
              <span className="font-bold text-[#1A2332] text-sm">٤.٩</span>
              <span className="text-[#5A6A72] text-sm">(١٢٠+ تقييم)</span>
            </div>
            <div className="w-px h-4 bg-[#D5E0DC]" />
            <span className="text-xs font-bold text-[#2D8B6F] bg-[#2D8B6F]/10 px-3 py-1 rounded-full">
              🔥 ٥٠٠+ طلب هذا الشهر
            </span>
            <div className="w-px h-4 bg-[#D5E0DC]" />
            <span className="text-xs font-bold text-[#0B6B5C]">
              ابدئي من ١٩٩ ريال
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {[
              { icon: CreditCard,  text: "الدفع عند الاستلام",   sub: "لا دفع مسبق" },
              { icon: ShieldCheck, text: "ضمان ذهبي ٣٠ يوم",    sub: "استرجاع كامل" },
              { icon: BadgeCheck,  text: "مصرح SFDA",            sub: "معايير صيدلانية" },
            ].map(({ icon: Icon, text, sub }) => (
              <div
                key={text}
                className="flex items-center gap-2 bg-white border border-[#D5E0DC] rounded-2xl px-4 py-2.5 shadow-sm"
              >
                <Icon size={16} className="text-[#0B6B5C] shrink-0" aria-hidden />
                <div className="text-right">
                  <p className="text-xs font-bold text-[#1A2332] leading-none">{text}</p>
                  <p className="text-[10px] text-[#5A6A72] mt-0.5">{sub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="container-padded">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <span className="font-bold text-[#1A2332] text-lg mr-1">٤.٩ من ٥</span>
            </div>
